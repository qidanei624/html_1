                ...
                <table class="typecho-list-table">
                    ...
                    <tbody>
                        ...
                        <tr<?php echo ($rs->pid % 2 == 0)?'':' class="even"'; ?> id="<?php echo 'threed-'.$rs->pid; ?>">
							...
							<td><?php echo (isset($rs->lottery_qh)) ? $rs->lottery_qh : '';?></td>
							<td><?php echo (isset($rs->num1)) ? get_prefix_nums($rs->lottery_qh, get_weishu_pm($rs->lottery_qh, $rs->num1,$status)):'';?></td>
							<td><?php echo (isset($rs->num1)) ? $rs->num1.' ['.get_weishu_pm($rs->lottery_qh, $rs->num1,$status).']' : '';?></td>
							<td><?php echo (isset($rs->num2)) ? get_prefix_nums($rs->lottery_qh, get_weishu_pm($rs->lottery_qh, $rs->num2,$status)):'';?></td>
							<td><?php echo (isset($rs->num2)) ? $rs->num2.' ['.get_weishu_pm($rs->lottery_qh, $rs->num2,$status).']' : '';?></td>
							<td><?php echo (isset($rs->num3)) ? get_prefix_nums($rs->lottery_qh, get_weishu_pm($rs->lottery_qh, $rs->num3,$status)):'';?></td>
							<td><?php echo (isset($rs->num3)) ? $rs->num3.' ['.get_weishu_pm($rs->lottery_qh, $rs->num3,$status).']' : '';?></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php else: ?>
                        <tr class="even">
