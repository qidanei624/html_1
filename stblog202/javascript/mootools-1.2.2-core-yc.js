...
new Native({
	...
	wrap: function(a,b,c) {
		...
		return function() {
			if(c._protected&&this._current==null) {throw new Error('The method "'+b+'" cannot be called.');
			}
	...
	Element.implement({
		...
		getOffsets: function() {
			if(Browser.Engine.trident) {
				...
				var m=d(this,"position")=="fixed";
				return {
					x:l.left+((m)?0:j.scrollLeft)-j.clientLeft,
					y:l.top+((m)?0:j.scrollTop)-j.clientTop
				};
			}
			var i=this,h= {
				x:0,
...
var Request=new Class({
	...
	send: function(j) {
		...
		var f=j.data,b=j.url,a=j.method;
		switch($type(f)) {
			case"element":
				f=$(f).toQueryString();
				break;
		...
		if(this.options.noCache) {
			var e="noCache="+new Date().getTime();
			f=(f)?e+"&"+f:e;
		}
		if(f&&a=="get") {
			b=b+(b.contains("?")?"&":"?")+f;
		...
		this.headers.each( function(l,k) {
			try {
				this.xhr.setRequestHeader(k,l);
			} catch(m) {
				this.fireEvent("exception",[k,l]);
			}
		},this);
